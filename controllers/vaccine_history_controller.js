...
class VaccineHisController {  
   ...
   async add_history(req, res, next) {
      const { id_user, id_vaccine, type_of_vaccine, birthday, address, date_vac} = req.body;
      // console.log({ name, gender, birthday, email, username, password });
      const userExists = await User.findOne({ id: id_user });
      if (userExists) {
            res.send({
      ...
      try {
         const vaccineHis = await VaccineHis.create({ id_user, id_vaccine, type_of_vaccine, birthday, address, date_vac});
   ...
   async get_vaccine_history_by_id(req, res, next) {
      ...
      try {
         const vaccineHis = await VaccineHis.find({ id_user: id });
         res.send({ "msg": 1, 'vaccineHis': vaccineHis });
      }
      catch (err) {
         res.status(401).send({
